...
\documentclass[11pt,oneside,titlepage]{optech}
...
\section{Statistics and Reliability}
...
\subsection{Basic introduction to statistical terminology}
...
\]
These two functions are respectively known as the probability mass function
(PMF) and cumulative distribution function (CDF).  Defining probability
functions for continuous random variables is a little more tricky, because
the probability of observing a particular value is zero.  We thus define a
probability density function (PDF), which is the limiting probability of
...
$[0,1]$.  There is a unique relationship between the PMF or PDF and the CDF,
...
a PMF, PDF or CDF, which will not all be described here, since they are well
...
\subsubsection{Multivariate probability distributions}
...
avoid it.  For discrete random variables the joint PMF and CDF can be
defined as:
\[
\begin{align}
...
possible to develop a conditional PMF:
\[
\begin{split}
...
integral over the PDF, which, if possible, is tedious mathematically and
costly computationally.

\subsubsection{Partial descriptors of random variables}
...
\subsubsection{The normal and multinormal distributions}
...
zero and a standard deviation of one.  The PDF and CDF are defined (using
...
The generalized form of the normal PDF and CDF are:
\[
\begin{align}
...
denote with $z$ throughout this discussion) then \textbf{M} is a zero matrix
and \textbf{D} is the identity matrix, and the PDF reduces to:

\[
\begin{array}{*{20}c}
   {\mathbf{{\rm Z}} \sim \operatorname{N} \left( {0,\mathbf{R}} \right)} & {\text{j}_n \left( {\mathbf{z},\mathbf{R}} \right) = \frac{{\left( {2\pi } \right)^{{{ - n} \mathord{\left/
 {\vphantom {{ - n} 2}} \right.
 \kern-\nulldelimiterspace} 2}} }}
{{\sqrt {\left| \mathbf{R} \right|} }}\exp \left( { - \frac{1}
{2}\mathbf{z}^\operatorname{T}  \mathbf{R}^{ - 1} \mathbf{z}} \right)}  \\
...
The lognormal distribution has two parameters: l and z, and the PDF and CDF
are defined as:
...
\subsubsection{Transformation to the standard normal space}
...
a marginal PDF $f(x)$ for each $x$ in our space $\mathbf{x}$, we need the
multivariate PDF $f(\mathbf{x})$, which would normally require the
...
Notice that only the marginal PDF of each of the random variables appears in
...
\subsubsection{Basic reliability concepts}
...
onto the PDF, and the integral of the area outside of this line taken.  In
...
\subsubsection{Multiple failure criteria}
...
\subsection{Integration using simulation techniques}
...
\subsubsection{Generation of random numbers}
...
marginal PDFs and correlation matrix, and so we need to transform our
...
\section{Layered Elastic Calculation Engine}
...
\subsection{Radial coordinate system}
...
\subsubsection{Determination of system response}
...
\subsubsection{Gaussian-Legendre Quadrature over products of Bessel functions}
...
\subsubsection{Load superposition algorithm}
...
\subsection{Calculation of Principal Stresses}
...
\subsubsection{Numerical solution of eigenvalues}
...
\subsubsection{Specialized solution for principal stresses}
