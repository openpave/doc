...
\begin{description}
\tightlist
\item[\textrm{Roman}] is used for general text.
\item[\textit{Italics}] are used for emphasis.
\item[\texttt{Typewriter}] is used for source code.
\end{description}
...
\begin{description}
\tightlist
\item[$\mathrm{Roman}$] is used for functions and operators.
\item[$\mathit{Italics}$] are used for scalars.
\item[$\mathbf{Bold}$] is used for vectors and matrices.
\item[$\mathtt{Typewriter}$] is used for puesdo-mathematics in code.
\item[$\mathsf{Sans-serif}$] is used for tensors.
\end{description}
...
normally only contain like objects.  The objects within a collection are
...
the set can be differentiated from one an other and not split into a smaller
item, and thus can be counted, but that there are an infinite number of items
in the set, or they can uncountable --- meaning that the items in the set can
always be split into smaller items, or the set can be finite.  In computer science
sets are always finite, since computers don't like dealing with infinite numbers.
...
having repeated items.

\subsection{Ordered Multisets}
...
is one in which the elements have been reordered according to some rule.
...
restrict the use of tuple to the database sense.
...
to as a tuple.  Within this document the term keyed set will be reserved for
sets where the key is a component of the value and the term map reserved for
sets where the key is a not a component.  By nature the key must be unique
(since otherwise one cannot find the items --- one cannot have two first
items in a list), and so a keyed set is naturally not capable of being a
multiset, while a map could have multiple equal values and thus be a multiset
(although if you consider the key and value tuple as a single object then a
map and keyed set are equivalent).
...
provide the concept of structures, which are a ordered tuple with named
...
closed form approximations for $\rho_{0,ij}$ as a function of \textit{ $\rho$
ij} and the distribution types of $x_i$ and $x_j$.
...
then be analyzed to draw conclusions about the behavior of the system.  The
...
$h(\mathbf{x})$, by centering $h(\mathbf{x})$ around the
points where $I(\mathbf{x})=1$.  This process can be repeated to
continue to improve $h(\mathbf{x})$.  If in addition to the
$I(\mathbf{x})$ values we also store the $g(\mathbf{x})$
...
\begin{equation*}
\begin{gathered}
  h(\mathbf{x}) = \sum_{j = 1}^m {p_j h_j (\mathbf{x})}  \hfill \\
  p_j  = \frac{{n_j }}
{{\sum_{j = 1}^m {n_j } }} \hfill \\ 
\end{gathered} 
\end{equation*}
...
stresses ($\sigma _x$, $\sigma _y$, $\sigma _z$), six shear stresses ($\tau
_{xy}=\tau _{yx}$, $\tau _{xz}=\tau _{zx}$, $\tau _{yz}=\tau _{yz}$), three
normal strains ($\epsilon _x$, $\epsilon _y$, $\epsilon _z$), six shear
strains ($\gamma _{xy}=\gamma _{yx}$, $\gamma _{xz}=\gamma _{zx}$, $\gamma
_{yz}=\gamma _{yz}$)and three components of displacement ($\Delta _x$,
$\Delta _y$, $\Delta _z$).  We have a $m$ number of circular loads, each at
...
are circular, the problem is therefore axi-symmetric around the center of
the load, and so we perform the calculations in a radial coordinate system.

In this radial coordinate system we also have three normal stresses ($\sigma
_r$,$\sigma _{\theta}$,$\sigma _z$), six shear stresses ($\tau
_{r\theta}=\tau _{\theta r}$,$\tau _{rz}=\tau _{zr}$,$\tau _{\theta z}=\tau
_{z\theta}$) and three components of displacement
($\Delta _r$,$\Delta _{\theta}$,$\Delta _z$).  These are shown in Figure 2.1.
...
\begin{equation*}
\begin{gathered}
  \sigma _z^i  = a\int_0^\infty  {m^2 J_1 \left( {ma} \right)J_0 \left( {mr} \right)\left( {\left( {1 - 2\upsilon _i } \right)\left( {B_i e^{mz}  + D_i e^{ - mz} } \right) - m\left( {\left( {A_i  + B_i z} \right)e^{mz}  - \left( {C_i  + D_i z} \right)e^{ - mz} } \right)} \right)dm}  \\ 
  \tau _{rz}^i  = a\int_0^\infty  {m^2 J_1 \left( {ma} \right)J_1 \left( {mr} \right)\left( {2\upsilon _i \left( {B_i e^{mz}  - D_i e^{ - mz} } \right) + m\left( {\left( {A_i  + B_i z} \right)e^{mz}  + \left( {C_i  + D_i z} \right)e^{ - mz} } \right)} \right)dm}  \\ 
  \Delta r^i  = \frac{{1 + \upsilon _i }}
{{E_i }}a\int_0^\infty  {mJ_1 \left( {ma} \right)J_1 \left( {mr} \right)\left( {B_i e^{mz}  + D_i e^{ - mz}  + m\left( {\left( {A_i  + B_i z} \right)e^{mz}  - \left( {C_i  + D_i z} \right)e^{ - mz} } \right)} \right)dm}  \\ 
  \Delta z^i  = \frac{{1 + \upsilon _i }}
{{E_i }}a\int_0^\infty  {mJ_1 \left( {ma} \right)J_0 \left( {mr} \right)\left( {2\left( {1 - 2\upsilon _i } \right)\left( {B_i e^{mz}  - D_i e^{ - mz} } \right) - m\left( {\left( {A_i  + B_i z} \right)e^{mz}  + \left( {C_i  + D_i z} \right)e^{ - mz} } \right)} \right)dm}  \\ 
  \sigma _r^i  = a\int_0^\infty  {m^2 J_1 \left( {ma} \right)J_0 \left( {mr} \right)\left( {\left( {1 + 2\upsilon _i } \right)\left( {B_i e^{mz}  + D_i e^{ - mz} } \right) + m\left( {\left( {A_i  + B_i z} \right)e^{mz}  - \left( {C_i  + D_i z} \right)e^{ - mz} } \right)} \right)dm}  \\ 
   - \frac{1}
{r}\frac{{E_i }}
{{1 + \upsilon _i }}\Delta r^i  \\ 
  \sigma _\theta ^i  = 2\upsilon _i a\int_0^\infty  {m^2 J_1 \left( {ma} \right)J_0 \left( {mr} \right)\left( {B_i e^{mz}  + D_i e^{ - mz} } \right)dm}  + \frac{1}
{r}\frac{{E_i }}
{{1 + \upsilon _i }}\Delta r^i  \\ 
\end{gathered} 
\end{equation*}
...
Quadrature to minimize the number of times we need to evaluate the system in
...
Using a generalized eigenvalue solution for a three by three stress tensor
...
The \ac{FEM} is now the \textit{de facto} method of solving differential equations in engineering.  It is used because it allows the solution to be expressed over a complex domain, which is discretized into 'elements' (which are termed finite elements since they have some real size, unlike the differentially small elements used in calculus).  The history and fundamentals of \ac{FEM} are covered in a number of textbooks, and so are not covered here.  The details which are covered here are for the solution of 3D linear elastic problems, such as those covered in the previous chapter.  There are three reasons for preferring \ac{FEM} to the analytic solution in the previous section:
\begin{enumerate}
  \tightlist
	\item the solution can be extended to non-linear visco-elasto-plastic materials;
	\item the thickness of the layers and other properties can be easily varied spatially, and
	\item the solution is faster if you are trying to analyze a large number of points (for contour plotting).
\end{enumerate}
...
The solution above does not detail the kind of elements used, because these can be varied.  In \OP\ implementation there are 3 types of elements available: an 8--node, a 16--node and a variable 16/34--node element.  The second two elements are not typical, and so are explained here.

\subsection{8--node brick element}

The 8--node brick element is the standard building block for 3D finite elements.  The element has linear shape functions in all three dimensions, so that it is only capable of representing a constant strain in the element.  By convention the shape functions are defined one a cube extending over the domain $[-1,1]\times[-1,1]\times[-1,1]$.  The 8 nodes are at the corners of this domain, so they have the following definitions:
\begin{align*}
N_1: & (-1,-1,-1) \\
N_2: & (-1,+1,-1) \\
N_3: & (+1,-1,-1) \\
...
In \OP\ code the $y$-axis is handled first, only because this enables better node numbering across the transverse direction in pavements.  This convention differs from most other treatments of the topic.  The shape functions are then defined as:
\begin{align*}
H_1(\xi,\eta,\zeta) = & \frac{(1-\xi)(1-\eta)(1-\zeta)}{8} \\
H_2(\xi,\eta,\zeta) = & \frac{(1-\xi)(1+\eta)(1-\zeta)}{8} \\
H_3(\xi,\eta,\zeta) = & \frac{(1+\xi)(1-\eta)(1-\zeta)}{8} \\
H_4(\xi,\eta,\zeta) = & \frac{(1+\xi)(1+\eta)(1-\zeta)}{8} \\
H_5(\xi,\eta,\zeta) = & \frac{(1-\xi)(1-\eta)(1+\zeta)}{8} \\
H_6(\xi,\eta,\zeta) = & \frac{(1-\xi)(1+\eta)(1+\zeta)}{8} \\
H_7(\xi,\eta,\zeta) = & \frac{(1+\xi)(1-\eta)(1+\zeta)}{8} \\
H_8(\xi,\eta,\zeta) = & \frac{(1+\xi)(1+\eta)(1+\zeta)}{8}
\end{align*}

\subsection{16--node brick element}

The 16--node brick element is a special element used in the \OP\ \ac{FEM} code.  It has linear shape functions in the horizontal plane and cubic shape functions in the vertical direction.  This enables it to capture the rapid changes in response with depth which are common in pavements, at the expense of some additional computations.  The node locations in the element space are:
\begin{align*}
   N_1: & (-1,-1,-1) \\
   N_2: & (-1,+1,-1) \\
   N_3: & (+1,-1,-1) \\
