...
approriate time in the text, the following general conventions are used
...
Within mathematical formulas the following convetions are generally used,
...
different names depending on their properties, and it is nessecary to
...
Mathmaticans, Computer Scientists and Database designers use the same terms
to refer to collections of objects, although their defitions often differ
...
collection is refered to as a container.
...
more detail in a later section.  However, a distiction must be drawn between
...
refered to as elements.

\subsection{Sets}

Sets are one of the most fundamental concepts in matematics and, in fact, all
...
pages of a book into the third.  In computer sience sets are always finite,
...
computer sience the same concept is often refered to as a bag.  Thus if you
...
distingished by having repeated items.  
...
tosses occured).  Tuples form the basis of vectors, tensors and matrix
...
In computer sience tuple is an overused term, which can mean an ordered
...
restrict the use of tuple to the computer sience sense.
...
ordered multiset are also distingushed by a subscripted index, such as $a_i$. 
...
mathematics --- most mathematical formulations are actually independant of
the order of the sets.  However, they are much more common in computer sience
where they are normally refered to as lists, or sometimes, unique lists.
...
abstract indices (or keys).  These types of sets are refered to as maps
...
Often in computer science the key and value pair from a map will be refered
